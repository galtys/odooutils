  ...
  config = lib.mkIf mainCFG.enable {
    ...
    systemd.services.run_in_loop2  = lib.mkIf mainCFG.run_in_loop {
      ...
      serviceConfig = {
          ...
          ExecStart = "${run_in_loop}/bin/run_in_loop2.py --pwd=${mainCFG.stateDir} --period=${mainCFG.period} --config-toml=${pjb_keyring}/pjb.toml --scripts-dir=${run_in_loop} ${mainCFG.dbx} ";};
     };
